...
const Header = () => {
  ...
  useEffect(() => {
    //all my user store updates this onAuthStateCHange will be called.
    const unsubcribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        //when user signout then we will dispatch another action removeUser()
        dispatch(removeUser());
        navigate("/");
      }
    });

    //unsubscribe when component unmounts to stop re-render
    return () => {
      unsubcribe();
    };
  }, []);
      ...
      {user && (
        <div className="flex p-2 justify-between">
         {showGPTSearch && <select className="p-2 bg-gray-500 text-white m-4" onChange={handleLanguageChange}>
          ...
          <button
            className="py-2 px-2 my-1 m-2 bg-purple-800 text-white rounded-lg"
